...
// 'bad+'/'bad-' removed — not real in-game feedback options.
...
interface HistoryIteration {
  ...
  feedback: { bias: FmBiasKey; value: number; feedback: FmFeedbackValue }[]
  applied_deltas: {
    lowestRuleBreak?: number
    possibleSetups?: number
    bestSetup?: number[] | null
  }
  created_at: string
}
...
export default function FmSetupsPage() {
  ...
  async function runCalculate(idx: 0 | 1, body: Record<string, any>) {
    ...
    try {
      ...
      updateLane(idx, {
        ...
        feedbackSelections: {},
        // A fresh calculation invalidates any previously-loaded history view
        // for this lane — the iteration count just changed underneath it.
        history: null,
        showHistory: false,
      })
  ...
  async function toggleHistory(idx: 0 | 1) {
    const lane = lanes[idx]
    const next = !lane.showHistory
    updateLane(idx, { showHistory: next })
    if (!next || lane.history || !lane.result?.session_id) return

    updateLane(idx, { historyLoading: true, historyError: '' })
    try {
      ...
      updateLane(idx, { history: data, markedOptimalAt: data.marked_optimal_at })
    } catch (err: any) {
  ...
  async function handleMarkOptimal(idx: 0 | 1) {
    const lane = lanes[idx]
    if (!lane.result?.session_id || !lane.result.best_setup) return
    updateLane(idx, { marking: true, markError: '' })
    try {
      ...
      updateLane(idx, { markedOptimalAt: new Date().toISOString() })
    } catch (err: any) {
      updateLane(idx, { markError: err.message })
    } finally {
      updateLane(idx, { marking: false })
    }
  }
                  ...
                  {lane.expanded && lane.result?.best_setup && (
                    <div className="rounded-2xl border border-rise-red/30 bg-rise-red/5 p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {lane.markedOptimalAt ? (
                          <span className="text-[11px] font-bold text-emerald-400 uppercase tracking-widest">✓ Confirmed Optimal</span>
                        ) : (
                          <button
                            onClick={() => handleMarkOptimal(idx)}
                            disabled={lane.marking}
                            className="text-[11px] font-bold uppercase tracking-widest text-rise-red disabled:opacity-40"
                          >
                            {lane.marking ? 'Marking…' : 'Mark as Optimal'}
                          </button>
                        )}
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                      ...
                      {lane.markError && <p className="text-red-400 text-[11px] mt-2">{lane.markError}</p>}
                    </div>
                  )}
                  ...
                  {lane.expanded && lane.result && (
                    <div>
                      <button
                        ...
                        className="text-white/30 text-[11px] uppercase tracking-widest underline"
                      >
                        {lane.showHistory ? 'Hide History' : 'Show History'}
                      </button>

                      {lane.showHistory && (
                        <div className="mt-2 rounded-2xl border border-white/10 bg-black/20 p-3 space-y-2">
                          {lane.historyLoading && <p className="text-white/30 text-xs">Loading…</p>}
                          {lane.historyError && <p className="text-red-400 text-xs">{lane.historyError}</p>}
                          {lane.history && lane.history.iterations.length === 0 && (
                            <p className="text-white/30 text-xs">No feedback submitted yet this session.</p>
                          )}
                          {lane.history?.iterations.map((it) => (
                            <div key={it.iteration_number} className="border-b border-white/5 last:border-0 pb-2 last:pb-0">
                              <p className="text-white/40 text-[10px] uppercase tracking-widest mb-1">
                                Iteration {it.iteration_number}
                                {it.applied_deltas?.lowestRuleBreak !== undefined && (
                                  <span> · {it.applied_deltas.lowestRuleBreak} break{it.applied_deltas.lowestRuleBreak === 1 ? '' : 's'}</span>
                                )}
                              </p>
                              <div className="flex flex-wrap gap-1.5">
                                {it.feedback.map((fb, i) => (
                                  <span
                                    key={i}
                                    className="text-[10px] px-2 py-1 rounded-full bg-white/5 text-white/60 border border-white/10"
                                  >
                                    {BIAS_LABELS[fb.bias]}: {FEEDBACK_LABELS[fb.feedback]}
                                  </span>
                                ))}
                              </div>
                            </div>
