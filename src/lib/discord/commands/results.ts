...
//
// PATCH (2026-09-14): resolveRound is now exported — results.ts needs it
// for explicit `round`-option lookups (matching by round_number or a
// name substring), while using its own default-resolution logic for the
// no-round-given case, since results wants the most recent COMPLETED
// round with recorded results, not the next upcoming race that
// resolveRound falls back to when roundInput is omitted.
 ...
 *
 * Exported so other command modules (e.g. results.ts) can reuse the
 * explicit-round-lookup half of this for their own `round` option,
 * without duplicating the number-vs-name matching logic. Callers that
 * need a different no-input default (results wants the most recent
 * COMPLETED round, not the next upcoming one) should branch before
 * calling this rather than relying on this function's default path.
 */
