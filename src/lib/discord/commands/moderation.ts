...
// ... existing discordModerationCall, kick, and ban registrations stay as-is ...
...
registerCommand("lockdown", async (ctx) => {
  ...
  if (!result.ok) {
    return { content: `Couldn't lock down the server: ${result.error}`, ephemeral: true };
  }
...
registerCommand("endlockdown", async (ctx) => {
  ...
  if (!result.ok) {
    return { content: `Couldn't lift the lockdown: ${result.error}`, ephemeral: true };
  }
