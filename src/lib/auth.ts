...
async function getRequestingDriver(supabase: any, session: any) {
  ...
  console.log('[getRequestingDriver] user keys:', Object.keys(user))
  console.log('[getRequestingDriver] discordId:', discordId)
  console.log('[getRequestingDriver] email:', email)
  ...
  if (discordId) {
    const { data } = await supabase
      ...
      .maybeSingle()
    console.log('[getRequestingDriver] discord lookup result:', data)
    driver = data
  }
  ...
  if (!driver && email) {
    const { data } = await supabase
      ...
      .eq('email', email)
      .maybeSingle()
    console.log('[getRequestingDriver] email lookup result:', data)
    driver = data
  }
...
async function hasStewwardAccess(
  ...
  // Check steward licence specifically for this league
  const { data: leagueLicence } = await supabase
    .schema('pitboss')
    .from('licences')
    .select('id')
    .eq('driver_id', driverId)
  ...
  if (leagueLicence) return true

  // Check driver_leagues role in this league
  ...
  if (leagueRole) return true

  // Check driver_leagues role in ANY league
