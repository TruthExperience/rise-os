...
import type { Corner, TelemetryFrame, TelemetryLap, TelemetrySession } from '@/lib/telemetry-types';
...
export interface CornerCoaching {
  ...
  /**
   * Coaching prose for this corner — generated by the LLM from this
   * corner's braking/throttle/issues data. Falls back to a deterministic,
   * template-built sentence (see buildDeterministicNarrative) if the LLM
   * call fails or omits this corner from its response, so the UI never
   * shows a blank note.
   */
  coachingNote: string;
}
...
export interface CoachingReport {
  ...
  summaryText: string;
  /** Whether summaryText / coachingNotes came from the LLM or the deterministic fallback. */
