...
// parser.ts — Top-level entry point. Peeks the header to determine spec year
// and packet type, then dispatches to the matching per-year parser.
// Only the 8 packet types we capture (Motion, Session, LapData, Participants,
// CarSetups, CarTelemetry, CarStatus, CarDamage) are wired up; anything else
// is ignored.
...
export type ParsedPacket =
  ...
  | { year: 23; kind: 'carSetups'; data: F23.PacketCarSetupData23 }
  ...
  | { year: 24; kind: 'carSetups'; data: F24.PacketCarSetupData24 }
  ...
  | { year: 25; kind: 'carSetups'; data: F25.PacketCarSetupData25 }
  ...
  | { year: 26; kind: 'carSetups'; data: F26.PacketCarSetupData26 }
...
function dispatch23(id: PacketId, buf: Buffer): ParsedPacket | null {
  switch (id) {
    ...
    case PacketId.CarSetups:
      return { year: 23, kind: 'carSetups', data: F23.parseCarSetups23(buf) };
...
function dispatch24(id: PacketId, buf: Buffer): ParsedPacket | null {
  switch (id) {
    ...
    case PacketId.CarSetups:
      return { year: 24, kind: 'carSetups', data: F24.parseCarSetups24(buf) };
    case PacketId.CarTelemetry:
...
function dispatch25(id: PacketId, buf: Buffer): ParsedPacket | null {
  switch (id) {
    ...
    case PacketId.CarSetups:
      return { year: 25, kind: 'carSetups', data: F25.parseCarSetups25(buf) };
    case PacketId.CarTelemetry:
...
function dispatch26(id: PacketId, buf: Buffer): ParsedPacket | null {
  switch (id) {
    ...
    case PacketId.CarSetups:
      return { year: 26, kind: 'carSetups', data: F26.parseCarSetups26(buf) };
    case PacketId.CarTelemetry:
...
export type { TelemetryYear };
 HEADER_SIZE_BYTES, PacketId, parseHeader, resolveTelemetryYear, TelemetryYear } from './header';
