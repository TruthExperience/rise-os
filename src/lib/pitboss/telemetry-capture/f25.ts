...
// Season Pack" variant (that one's a separate spec — see f26.// f25.ts — F1 25 packet body parsers (Motion, Session, LapData,
...
// F1 25 UDP spec (https://github.com/MacManley/f1-25-udp), which itself
// mirrors
// https://forums.ea.com/blog/f1-games-game-info-hub-en/ea-sports%E2%84%A2-f1%C2%AE25-udp-specification/12187347.
// Note: this is the base 2025 packet format, not the mid-cycle "2026
...
//   - Motion, Session, LapData, CarSetups, CarTelemetry, CarStatus:
//     unchanged from F24 — re-exported here under the F25 name/type for
//     the dispatch table in parser.ts, but byte-for-byte identical
//     logic. (CarSetups stays at 1133 bytes total, incl. engineBraking
//     and the trailing nextFrontWingValue introduced in F24.)
...
// + 4 (nextFrontWingValue). Unchanged from F24 (struct, offsets, and
// trailing nextFrontWingValue field all byte-for-byte identical).
// ---------------------------------------------------------------------

export interface CarSetupData25 {
  frontWing: number;
  rearWing: number;
  onThrottle: number;
  offThrottle: number;
...
export interface PacketCarSetupData25 {
  header: PacketHeader;
  carSetups: CarSetupData25[];
  /** Value of front wing after next pit stop — player's own car only; 0 for other cars. */
  nextFrontWingValue: number;
}

function readCarSetupData25(r: Reader): CarSetupData25 {
  return {
    frontWing: r.u8(),
    rearWing: r.u8(),
    onThrottle: r.u8(),
...
export function parseCarSetups25(buf: Buffer): PacketCarSetupData25 {
  const header = parseHeader(buf);
  const r = new Reader(buf, HEADER_SIZE_BYTES);
  const carSetups: CarSetupData25[] = [];
  for (let i = 0; i < NUM_CARS; i++) carSetups.push(readCarSetupData25(r));
