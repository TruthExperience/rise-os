...
//   https://forums.ea.com/blog/f1-games-game-info-hub-en/ea-sports%E2%84%A2-f1%C2%AE25-udp-specification/// f26.ts — F1 26 packet body parsers (Motion, Session, LapData,
// Participants, CarSetups, CarTelemetry, CarStatus, CarDamage). Field
// layouts and byte offsets verified against EA's official "2026 Season
// Pack Telemetry Output Structures" spec, published alongside the base
// F1 25 spec for direct comparison:
...
//   - CarSetups: struct unchanged from F24/F25 (still 50 bytes/car +
//     trailing nextFrontWingValue) — only benefits from the NUM_CARS
//     22->24 change, same pattern as LapData/CarDamage below.
...
// ---------------------------------------------------------------------
// CarSetups — 1233 bytes total = 29 (header) + 24 * 50 (CarSetupData)
// + 4 (nextFrontWingValue). Struct unchanged from F24/F25 — only
// NUM_CARS (22->24) differs, same pattern as LapData/CarDamage above.
// ---------------------------------------------------------------------

export interface CarSetupData26 {
  frontWing: number;
  rearWing: number;
  onThrottle: number;
  offThrottle: number;
...
export interface PacketCarSetupData26 {
  header: PacketHeader;
  carSetups: CarSetupData26[];
  /** Value of front wing after next pit stop — player's own car only; 0 for other cars. */
  nextFrontWingValue: number;
}

function readCarSetupData26(r: Reader): CarSetupData26 {
  return {
    frontWing: r.u8(),
    rearWing: r.u8(),
    onThrottle: r.u8(),
...
export function parseCarSetups26(buf: Buffer): PacketCarSetupData26 {
  const header = parseHeader(buf);
  const r = new Reader(buf, HEADER_SIZE_BYTES);
  const carSetups: CarSetupData26[] = [];
  for (let i = 0; i < NUM_CARS; i++) carSetups.push(readCarSetupData26(r));
...
// Every struct below was cross-checked by summing its field sizes and
// confirming the result matches EA's documented total packet size,
// exactly the fsame rigor as f25.ts (each total below matched EA's
// stated size on the first attempt except CarTelemetry, which is why
// its engineTemperature width change was caught).
//
// Confirmed changes vs F25 (every other field, in every other struct,
