...
// Lockout durations per attempt (hours)
...
export async function POST(req: NextRequest) {
  ...
  if (!answers || typeof answers !== 'object' || Array.isArray(answers)) {
    return NextResponse.json({ error: 'answers must be an object mapping question_id to selected option' }, { status: 400 })
  }

  // ── Load certification — must belong to this driver and be in_progress ────────
  ...
  const score          = Math.round((correctCount / totalQuestions) * 100 * 100) / 100 // 2dp
  const passed         = score >= Number(cert.pass_mark)
  const now            = new Date()

  // ── Compute lockout if failed ─────────────────────────────────────────────────
  ...
  // ── On pass: update driver_leagues + auto-issue licence if not exists ─────────
  if (passed) {
    // Mark driver_leagues as certified
    ...
    if (dlError) {
      // Non-fatal — cert is already written as passed, log and continue
      console.error('[POST /api/pitboss/cert/submit] driver_leagues update', dlError)
    }

    // Check if a licence already exists for this driver + league
